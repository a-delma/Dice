...
(* Check if a certain kind of binding has void type or is a duplicate
  ...
  let rec check_type env ty = match ty with 
    ...
    let create_struct env (sname, binds) = 
      ...
      | true ->  
          let bind_name env' (ty, name) = StringMap.add name 
                                            (check_type env ty) env' in
    ...
    let struct_indices = 
      let map_func struc =
        ...
        in let (new_indices, _) = StringMap.fold fold_func struc 
                                    (StringMap.empty, 0) in 
      new_indices 
  ...
  let lookup_field_type typ name = match typ with 
      TypVar(typname) -> 
        ...
        in (try StringMap.find name binds 
            with _ -> raise (Failure ("Struct " ^ typname ^ 
                                        " does not have field " ^ name))) 
    | _ -> raise (Failure ("Type " ^ string_of_typ typ ^ 
  ...
  let rec expr envs expression = match expression with
    ...
    | Binop(e1, op, e2) as e -> 
      ...
      let nullComparison = (t1 = Void && is_not_primitive t2) || 
                                          (t2 = Void && is_not_primitive t1) in
      let same = t1 = t2 in
      let bothNum = ((t1 = Int)||(t1 = Float))&&((t2 = Int)||(t2 = Float)) in
      let bothBoollike = ((t1 = Bool) || (t1 = Float)) && ((t2 = Bool) || 
                                                                (t2 = Float)) in
        ...
        Failure ("Illegal binary operator " ^
          ...
          let finalSB = if same
            then (ty, SBinop((t1, e1'), op, (t2, e2')))
                  ...
                  then if t1 = Void
                      then (ty, SBinop((t2, SNullPointerCast(t2, 
                                                  (Void, e1'))), op, (t2, e2')))
                      else (ty, SBinop((t1, e1'), op, (t1, SNullPointerCast(t1, 
                                                                 (Void, e2')))))
                  else if t1 = Int
                      then (ty, SBinop((Float, SCall((Arrow([Int], Float), 
                               SId "intToFloat"), [(t1, e1')])), op, (t2, e2')))
                      else if t2 = Int 
                            then (ty, SBinop((t1, e1'), op, (Float, SCall(
                                        (Arrow([Int], Float), 
                                         SId "intToFloat"), [(t2, e2')]))))
                            else if t1 = Bool
                                then (ty, SBinop((t1, e1'), op, (Bool, SCall(
                                        (Arrow([Float], Bool), 
                                         SId "floatToBool"), [(t2, e2')]))))
                                else (ty, SBinop((Bool, SCall(
                                        (Arrow([Float], Bool), 
                                         SId "floatToBool"), [(t1, e1')])), op, 
                                                                     (t2, e2')))
          in finalSB
    | Assign(le, re) -> (match le with 
        Id(_) | RecordAccess(_, _) ->
          ...
          if lt = rt
            ...
            else raise (Failure ("Expected equal types but found " 
    ...
    | AssignList(typ, assigns) -> 
      let get_checked_bind = function 
        (name, e) -> let (sexpr_type, sx) as sexpr = expr envs e in
                     ...
                     then (name, (field_type, SNullPointerCast(field_type, 
                                                            (sexpr_type, sx))))
                     else raise (Failure ("Field " ^ name ^ " of type " ^ 
                                          string_of_typ typ ^ " has type " ^ 
                                          string_of_typ field_type ^ 
                                            " but expression " ^ 
    ...
    | Call(callable, args) as call -> 
      ...
      (match func_type with
        Arrow((param_types, return_type)) ->
          ...
          else raise (Failure ("Expecting " ^ string_of_int param_count ^ 
                                        " arguments in " ^ string_of_expr call))
        | _ -> raise (Failure ("Type " ^ string_of_typ func_type ^ 
      ...
      let field_ty = (match ev_expr_ty with
        TypVar name -> 
          let this_stuct = try StringMap.find name struct_env 
                           with _ -> raise (Failure (name ^ 
                                             " is not a valid struct type.")) in
          let this_ty    = try StringMap.find field this_stuct 
                          with _ -> raise (Failure (field ^ 
                                          " is not a field of type " ^ name)) in
            this_ty
        | _ -> raise (Failure ("Invalid struct access on type " ^ 
                                                   (string_of_typ ev_expr_ty))))
    ...
    | Lambda l         -> 
      ...
      let local_env = (List.fold_left (fun m (ty, name) -> StringMap.add 
                                                                      name ty m)
                      StringMap.empty 
                      locals') in 
    ...
    in if not ((t' = Bool) || (t' = Float)) 
      then raise (Failure err)
           ...
           then (Bool, SCall((Arrow([Float], Bool), SId "floatToBool"), 
                                                                    [(t', e')]))
           else (t', e') 
  ...
  and check_stmt envs statement = match statement with
    ...
    | If(p, b1, b2) -> SIf(check_bool_expr envs p, check_stmt envs b1, 
                                                            check_stmt envs b2)
    | For(e1, e2, e3, st) ->
        SFor(expr envs e1, check_bool_expr envs e2, expr envs e3, 
                                                            check_stmt envs st)
