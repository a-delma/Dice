...
eval evaluates one sx and will often be a pattern matching function with a large wildcard.
...
let fold_tree (eval : sx -> 'a) (zipper : 'a -> 'a -> 'a) 
              (empty : 'a)      (root_stmt : sstmt)     = 
  ...
  and fold_expr ex = zipper (eval ex) (match ex with
    ...
    | SLambda(l)          -> fold_stmt (SBlock l.sbody)
    | _                   -> empty)
