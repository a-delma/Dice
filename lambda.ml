...
let rec is_return = function
    SIf(_, s1, s2) -> (true, snd (check_stmt_for_return s1) && 
                          snd(check_stmt_for_return s2))
  | SWhile(_,_)    -> (true, false)
  | SReturn(_)     -> (false, true)
  | _              -> (false, false) 

and zipper (disregard_subexpression, root_has_return) (_, subexpr_has_return) =
    if disregard_subexpression
    then (false, root_has_return)
    else (false, root_has_return || subexpr_has_return)

and check_stmt_for_return stmt = fold_tree_with_stmt is_return (fun _ -> 
                                (false, false)) zipper (false, false) false stmt
  ...
  (match sl.st with 
    ...
    | _    -> let (_, has_return) = check_stmt_for_return (SBlock sl.sbody) in
                ...
                then true
                              ...
                              ("Not every path returns a value in " ^ sl.sid)))
            
and return_pass (sls : sLambda list) = List.fold_right (fun l acc -> 
      (check_for_return l) && acc) sls true