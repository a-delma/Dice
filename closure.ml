...
(* Returns a tuple with the first element indicating if it is a global/local 
   or not, and second element indicating the type no matter where it is.
   Errors if not found *)
and location_and_type s = (match envs with 
  local :: prev_and_global -> 
                      if StringMap.mem s local
...
and eval = function
  ...
  | SAssign(e1, _)       -> (match e1 with
    (_, SId(s)) -> let (loc, _) = location_and_type s in
                   if loc
                   then []
                   else raise (Failure ("Attempt to reassign to \"" ^ s ^
                                          "\" from closure"))
    | _ -> [])
  | SLambda(l) -> let locals = (match envs with
                                    (locals::_) -> locals
                                  | [] -> raise (Failure ("Local environment" ^
                                                          " is absent"))) in 
  (*Returns the list l_super minus any elements that are in m_sub*)
                  let rec diff l_super m_sub = (match l_super with
                      ((t, s)::tail) -> (try if t = StringMap.find s m_sub
                                             then diff tail m_sub
                                             else raise (Failure ("Shadowing " ^
